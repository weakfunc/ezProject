...
我有一个激光振镜控制项目，需要在STM32上实现对激光点XY坐标的控制。
硬件：

STM32主控
两个ZDT X42S闭环步进电机，通过串口控制
YAW轴电机控制屏幕左右，PITCH轴电机控制屏幕上下

坐标系：

屏幕中央为原点，右为+X，上为+Y，单位：度（°）
YAW工作零点=45°，PITCH工作零点=35°（此时光斑在屏幕中央）
转换公式：θ_yaw = 45 - screen_x，θ_pitch = 35 - screen_y
软件限位：screen_x和screen_y均限制在±15°以内

当前目标：
实现一个STM32固件，实现func_gambal模块的FUNC_GAMBAL_Updata(user1TaskInfo.yawRef, user1TaskInfo.picthRef);，debug变量输入屏幕坐标（度），自动转换并驱动两个电机到对应角度。包含：
X42S上电回零流程
laser_goto坐标转换与发送
软件限位保护

注意：先阅读driver_stepperMotor模块中代码，电机实际控制通过重写 控制弱函数DRIVER_STEPPER_Axis1CtrlHook和DRIVER_STEPPER_Axis2CtrlHook实现，在FUNC_GAMBAL_Updata中调用DRIVER_STEPPER_Update。 要不会有时序问题
