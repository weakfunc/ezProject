...
- **项目名称**（显示在 APP 主页顶部）：**两轮电动车智能防盗与定位系统设计**
- **项目作者**（显示在 APP 主页顶部）：2200810602陈敏敏

> 以下为可选补充，留空则由 Claude 自动设计：

```
APP端需要额外处理stm32发来的原始GPS数据，来完成实时位置查看，历史运动轨迹回放功能。原则是实现功能即可，方案任意，简单优先
```
...
```
声光报警通过板载的蜂鸣器和LED灯实现，项目的driver层已实现相关API。OLED上显示当前GPS信息：GPS状态，GPS经纬度坐标（变量部分使用大字体，如：经纬度的具体变量值），休眠倒计时（30s）。stm32进入休眠后通过任意KEY按键按下唤醒。上述功能API均已实现。
GPS信息更新周期1s。对于实时位置，历史运动轨迹回放等功能在上位机APP端实现，stm32只负责周期发送gps数据，不做gps数据处理。
对于防盗响应的震动监测功能，当防盗功能开启时，使用陀螺仪的加速度计等信息融合判断是否震动，在stm32端处理，stm32向app提供是否报警标志位。
对于防盗响应的位移监测功能，当防盗功能开启时，位移超过10m，则报警。
```
