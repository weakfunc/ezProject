...
/* 72MHz主频下WS2812B等待周期数，由示波器实测校准。
 * STM32F103 flash 2 wait states 导致 CYCCNT 捕获存在固定开销：
 *   HIGH 相位开销 ≈ 21cyc（297ns），LOW 相位开销 ≈ 27cyc（375ns）
 * 等待周期 = 目标时长对应周期数 - 固定开销
 * T0H=400ns→wait=7cyc,  T0L=850ns→wait=34cyc
 * T1H=800ns→wait=36cyc, T1L=450ns→wait=6cyc
 * Reset: >50us→3600cyc */
static const uint32_t ws2812Bit0HighCyc = 7U;
static const uint32_t ws2812Bit0LowCyc  = 34U;
static const uint32_t ws2812Bit1HighCyc = 23U;
static const uint32_t ws2812Bit1LowCyc  = 4U;
