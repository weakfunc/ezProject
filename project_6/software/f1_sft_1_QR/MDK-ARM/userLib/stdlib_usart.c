...
typedef struct {
  ...
  uint8_t rawTxBuf[RAW_TX_BUF_SIZE];    /* Raw TX buffer used by async UART send. */
...
void STDLIB_USART_SendRaw(uint8_t port, uint8_t *data, uint16_t len){
  ...
  uartRawInfo_t *c = &uartRawInfo[port];
  if(len > RAW_TX_BUF_SIZE) return;
  if(!__STDLIB_USART_WaitTxReady(c->huart)) return;

  for(uint16_t i = 0U; i < len; i++){
    c->rawTxBuf[i] = data[i];
  }
  __STDLIB_USART_TxSend(c, c->rawTxBuf, len);
}
