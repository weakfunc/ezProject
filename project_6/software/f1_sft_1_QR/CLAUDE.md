...
## 任务
  新建func_leadScrew模块，该模块是滑动丝杆机构，使用driver_stepperMotor中步进电机圈数控制API驱动。默认使用电机地址0x01。
  现已测量知：丝杆导程2mm，即步进电机顺时针（速度>0）转动5圈，丝杆执行器向负方向移动1cm
  实现：
  丝杆导程可宏定义配置，默认2mm
  丝杆距离控制（输入参数：丝杆相对原点距离，单位cm，正值为正方向移动，负值为负方向移动）
  丝杆状态查询（周期调用，把丝杆状态（运行中，已到达指定位置，等待参数输入）储存在leadScrewInfo中，同时把丝杆当前相对于0点位置储存在leadScrewInfo中）
  丝杆原点标定（无输入值无返回值，调用该API则定义此时丝杆位置为零点）
