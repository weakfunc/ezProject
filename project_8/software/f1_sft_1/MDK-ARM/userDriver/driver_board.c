...
/* 消抖阈值：连续检测到按下的采样次数，达到后才注册上升沿（10ms×2=20ms） */
#define BOARD_KEY_DEBOUNCE_CNT    (3U)

/* 按键私有追踪状态 */
typedef struct {
    uint8_t  lastPressed;       /* 上次是否处于按下状态（消抖后的确认状态） */
    uint32_t pressStartTickMs;  /* 按下起始时刻（消抖确认时记录） */
    uint8_t  longPressCounted;  /* 本次按下是否已触发长按计数（KeyInfoUpdate专用） */
    uint8_t  debounceConsecCnt; /* 连续采样到按下的次数，用于消抖 */
} boardKeyState_t;
...
void DRIVER_BOARD_KeyInit(void){
    ...
    for(uint8_t i = 0U; i < BOARD_KEY_COUNT; i++){
        ...
        boardKeyState[i].debounceConsecCnt = 0U;
...
void DRIVER_BOARD_KeyInfoUpdate(void){
    ...
    for(uint8_t i = 0U; i < BOARD_KEY_COUNT; i++){
        ...
        if(isPressed != 0U){
            ...
            /* 消抖：连续采样到低电平才递增计数器，不超过阈值上限 */
            if(boardKeyState[i].debounceConsecCnt < BOARD_KEY_DEBOUNCE_CNT){
                boardKeyState[i].debounceConsecCnt++;
            }

            /* 上升沿检测：消抖计数达到阈值后才注册按下事件，防止毛刺误触 */
            if((boardKeyState[i].lastPressed == 0U) &&
               (boardKeyState[i].debounceConsecCnt >= BOARD_KEY_DEBOUNCE_CNT)){
