...
/*============================================================================
 * README
 * 在周期任务的结尾调用DRIVER_OLED_Refresh();才会刷新屏幕
 *============================================================================*/
...
/* 以2倍缩放显示单个6x8字符，实际输出12x16像素（每个源像素扩展为2×2）。
 * 直接复用 asc2_0806 字库，无需独立字模表。
 * x：列起始坐标；y：行起始坐标；chr：ASCII字符；mode：1=正显，0=反显
 */
static void __DRIVER_OLED_ShowChar12x16(uint8_t x, uint8_t y, char chr, uint8_t mode){
  uint8_t chrOffset;
  uint8_t temp;
  uint8_t px;
  ...
  for(uint8_t col = 0U; col < 6U; col++){
    temp = asc2_0806[chrOffset][col];
    for(uint8_t row = 0U; row < 8U; row++){
      /* 根据字模位和mode决定像素亮灭 */
      px = ((temp & 0x01U) != 0U) ? mode : (uint8_t)(mode == 0U ? 1U : 0U);
      /* 每个源像素向右、向下各扩展1倍，形成2×2目标像素块 */
      DRIVER_OLED_DrawPoint((uint8_t)(x + col * 2U),      (uint8_t)(y + row * 2U),      px);
      DRIVER_OLED_DrawPoint((uint8_t)(x + col * 2U + 1U), (uint8_t)(y + row * 2U),      px);
      DRIVER_OLED_DrawPoint((uint8_t)(x + col * 2U),      (uint8_t)(y + row * 2U + 1U), px);
      DRIVER_OLED_DrawPoint((uint8_t)(x + col * 2U + 1U), (uint8_t)(y + row * 2U + 1U), px);
      temp >>= 1U;
    }
  }
}

/* 以2倍缩放显示字符串，每字符实际占12x16像素，仅更新显存 */
void DRIVER_OLED_ShowString12x16(uint8_t x, uint8_t y, const char *str){
  uint8_t     xPos = x;
  const char *p    = str;

  while(*p != '\0'){
    __DRIVER_OLED_ShowChar12x16(xPos, y, *p, 1U);
    xPos = (uint8_t)(xPos + 12U);
    p++;
  }
}

/* 以2倍缩放显示浮点数，每字符实际占12x16像素，decLen为小数位数，返回下一个x坐标 */
uint8_t DRIVER_OLED_ShowFloat12x16(uint8_t x, uint8_t y, float val, uint8_t decLen){
  ...
  /* 处理负数 */
  if(val < 0.0f){
    __DRIVER_OLED_ShowChar12x16(xPos, y, '-', 1U);
    xPos = (uint8_t)(xPos + 12U);
    val  = -val;
  }
  ...
  /* 显示整数部分 */
  for(uint8_t i = 0U; i < intLen; i++){
    uint8_t digit = (uint8_t)((intPart / __DRIVER_OLED_Pow(10U, (uint8_t)(intLen - i - 1U))) % 10U);
    __DRIVER_OLED_ShowChar12x16(xPos, y, (char)('0' + digit), 1U);
    xPos = (uint8_t)(xPos + 12U);
  }
  ...
  if(decLen > 0U){
    __DRIVER_OLED_ShowChar12x16(xPos, y, '.', 1U);
    xPos = (uint8_t)(xPos + 12U);
    for(uint8_t i = 0U; i < decLen; i++){
      uint8_t digit = (uint8_t)((decPart / __DRIVER_OLED_Pow(10U, (uint8_t)(decLen - i - 1U))) % 10U);
      __DRIVER_OLED_ShowChar12x16(xPos, y, (char)('0' + digit), 1U);
      xPos = (uint8_t)(xPos + 12U);
    }
  }
