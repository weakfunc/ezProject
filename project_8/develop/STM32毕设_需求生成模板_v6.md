...
```
用户按下KEY1 oled更新当前测温结果，按下KEY2 阈值+0.1°， 按下KEY3 阈值-0.1°。注意，读取温度的API是周期更新的，按下KEY1时只是把最新的温度数据显示到OLED上，未按下时oled维持上一次的值。蜂鸣器和RGB报警也要实现。OLED也需要显示电池图标，使用ADC1采集到的电压值转换为百分比
```
