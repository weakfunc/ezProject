...
typedef struct {
  uint8_t  addr;            /* 电机串口地址 */
  int32_t  realPos_01deg;  /* 实时位置，单位 0.1°，已含符号（X固件）*/
  int16_t  realSpd_01rpm;  /* 实时转速，单位 0.1RPM，已含符号（X固件）*/
  uint16_t encoder;        /* 线性化编码器值，0-65535 对应 0-360° */
  int32_t  posErr_001deg;  /* 位置误差，单位 0.01°，已含符号（X固件）*/
 ...
 * speed: 目标速度（0.1RPM），范围 0-30000（即 0-3000.0RPM）
 * sync : 同步标志
 */
 ...
 * maxSpeed: 最大速度（0.1RPM），范围 0-30000
 * pos     : 位置角度（0.1°），范围 0-4294967295
 * mode    : 运动模式（STEPPER_MODE_*）
 * sync    : 同步标志
 */
