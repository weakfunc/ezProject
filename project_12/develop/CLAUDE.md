...
## 任务
 我在根目录的otherProject文件夹中放置了一个物流分拣系统的项目工程，我需要你把该工程下task_user1中实现的功能完整移植到当前工程的task_user1中。OLED部分使用当前工程，再增加显示一个系统时间。这两个工程的主要差异就是摄像头发送数据的协议格式改变。现在需要用当前工程的新协议格式适配原有舵机分拣的功能。
