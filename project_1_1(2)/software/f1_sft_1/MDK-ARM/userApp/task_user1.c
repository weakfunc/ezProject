...
static void packRecognize(void){
  ...
  packList[sysCtrl.packNum].aim       = k210RxData;
  ...
  for(i = 0U; i < STEER_SERVO_COUNT; i++){
    if(sysCtrl.servoEnable[i] == 1U){
			if(i == STEER_SERVO_1){
				...
				DRIVER_STEER_Rotate360(STEER_SERVO_2, STEER_DIR_CW,  400); /* 顺时针旋转500ms（拨开包裹） */
				DRIVER_STEER_Rotate360(STEER_SERVO_2, STEER_DIR_CCW, 380); /* 逆时针旋转500ms（回到原位） */
			}
