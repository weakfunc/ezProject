...
- 新建driver_tjcLCD模块，该模块是TJC的串口屏模块，默认使用USART3
- ### 数据帧格式（固定16字节）
...
| [1]      | 0xAA        | 包头2                |
| [2]      | 0x05~0x08   | 控制字段             |
| [3]~[12] | ASCII字符串 | 数据内容，不足补0x00 |
| [13]     | 0x00        | CRC占位（不校验）    |
| [14]     | 0x00        | CNT占位              |
| [15]     | 0xFF        | 包尾                 |

STM32需要回传状态更新屏幕上的文本控件，格式为：控件名.txt="内容" + 三个0xFF结尾。编码为GB2312。
留出控件名和对应内容的接口，以下是示例，不需要具体实现，仅做这样格式的接口
eg：
// "雨声"的GB2312编码：D3 EA C9 F9
// 完整发送：t3.txt="雨声" + FF FF FF
