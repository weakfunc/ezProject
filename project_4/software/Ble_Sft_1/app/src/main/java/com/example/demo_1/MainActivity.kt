...
fun HomeScreen(
    ...
    val hasBleConnection = connectedDevice != null
    ...
    val reportedTargetTemp = (frame18?.var4b2 ?: 40).coerceIn(35, 48)
    val reportedMassageModeRaw = frame18?.var1b1 ?: 0
    val reportedMassageMode = reportedMassageModeRaw.coerceIn(0, 3)
    val statusByte = frame18?.var1b2 ?: 0
    val isRunning = hasBleConnection && (statusByte and 0x01) != 0
    val isHeating = hasBleConnection && (statusByte and 0x02) != 0
    val isDryBurn = hasBleConnection && (statusByte and 0x04) != 0
    val isOverTemp = hasBleConnection && (statusByte and 0x08) != 0
    val remainSec = if (hasBleConnection) (frame19?.var4b1 ?: 0).coerceAtLeast(0) else 0
    val faultCode = if (hasBleConnection) (frame19?.var1b1 ?: 0) else 0

    val modeNames = listOf("停止", "轻柔", "标准", "强劲")
    val currentModeText = if (hasBleConnection) {
        modeNames.getOrElse(reportedMassageModeRaw) { "未知" }
    } else {
        "--"
    }
    val remainTimeText = when {
        !hasBleConnection -> "--"
        remainSec > 0 -> String.format(Locale.US, "%02d:%02d", remainSec / 60, remainSec % 60)
        else -> "未设定"
    }
    ...
    var targetTempState by remember { mutableStateOf(40) }
    var massageModeState by remember { mutableStateOf(0) }
    var timerMinState by remember { mutableStateOf(0) }
    var powerState by remember { mutableStateOf(0) }

    LaunchedEffect(connectedDevice?.address, frame18?.cnt) {
        if (!hasBleConnection) return@LaunchedEffect
        targetTempState = reportedTargetTemp
        massageModeState = reportedMassageMode
        powerState = if (isRunning) 1 else 0
    }

    LaunchedEffect(connectedDevice?.address, frame19?.cnt) {
        if (!hasBleConnection) return@LaunchedEffect
        if (timerMinState == 0 && remainSec > 0) {
            timerMinState = ((remainSec + 59) / 60).coerceIn(0, 60)
        }
    }

    fun sendControlFrame(
        targetTemp: Int = targetTempState,
        timerMin: Int = timerMinState,
        massageMode: Int = massageModeState,
        powerOn: Int = powerState
    ) {
        val frame = BleProtocol.buildTxFrame(
            cmd = 0x21,
            var4_1 = targetTemp.coerceIn(35, 48),
            var4_2 = timerMin.coerceIn(0, 60),
            var1_1 = massageMode.coerceIn(0, 3),
            var1_2 = if (powerOn == 0) 0 else 1
        )
    ...
    fun updateTargetTemp(nextTargetTemp: Int) {
        val clampedTemp = nextTargetTemp.coerceIn(35, 48)
        targetTempState = clampedTemp
        sendControlFrame(targetTemp = clampedTemp)
    }

    fun updateMassageMode(nextMassageMode: Int) {
        val clampedMode = nextMassageMode.coerceIn(0, 3)
        massageModeState = clampedMode
        sendControlFrame(massageMode = clampedMode)
    }

    fun updateTimerMin(nextTimerMin: Int) {
        val clampedTimer = nextTimerMin.coerceIn(0, 60)
        timerMinState = clampedTimer
        sendControlFrame(timerMin = clampedTimer)
    }

    fun togglePowerState() {
        val nextPowerState = if (powerState == 0) 1 else 0
        powerState = nextPowerState
        sendControlFrame(powerOn = nextPowerState)
    }
        ...
        Column(
            ...
            HomeSection(title = "系统状态") {
                Column(
                    ...
                    ProtoFieldRow(
                        label1 = "当前水温",
                        value1 = if (hasBleConnection) {
                            "${String.format(Locale.US, "%.1f", currentTemp)}℃"
                        } else {
                            "--"
                        },
                        label2 = "目标温度",
                        value2 = if (hasBleConnection) "$reportedTargetTemp℃" else "--"
                    )
                    HorizontalDivider(color = MaterialTheme.colorScheme.outlineVariant)
                    ProtoFieldRow(
                        label1 = "加热状态",
                        value1 = if (hasBleConnection) {
                            if (isHeating) "加热中" else "未加热"
                        } else {
                            "--"
                        },
                        label2 = "系统状态",
                        value2 = if (hasBleConnection) {
                            if (isRunning) "运行中" else "待机"
                        } else {
                            "--"
                        }
                    )
                    ...
                    ProtoFieldRow(
                        label1 = "剩余定时",
                        value1 = remainTimeText,
                        label2 = "当前模式",
                        value2 = currentModeText
                    )
                }
            }

            HomeSection(title = "温度设定") {
                Column(
                    modifier = Modifier
                        .fillMaxWidth()
                        .padding(horizontal = 16.dp, vertical = 14.dp),
                    ...
                    Text(
                        text = "设定温度：" + if (hasBleConnection) "$targetTempState℃" else "--",
                    ...
                    Row(
                        ...
                        Button(
                            onClick = { updateTargetTemp(targetTempState - 1) },
                    ...
                    Row(
                        ...
                        listOf(35, 38, 40).forEach { temp ->
                            SelectableActionButton(
                                text = "${temp}℃",
                                selected = targetTempState == temp,
                                modifier = Modifier.weight(1f),
                                onClick = { updateTargetTemp(temp) }
                            )
                        }
                    }
                    Row(
            ...
            HomeSection(title = "按摩模式") {
                Column(
                    ...
                    Row(
                        ...
                        modeNames.forEachIndexed { index, label ->
                            SelectableActionButton(
                                text = label,
                                selected = massageModeState == index,
                                modifier = Modifier.weight(1f),
                                onClick = { updateMassageMode(index) }
                            )
                        }
                    }
                }
            ...
            HomeSection(title = "定时设置") {
                Column(
                    ...
                    Text(
                        text = "定时时长：" + if (hasBleConnection) {
                            if (timerMinState > 0) "${timerMinState}分钟" else "未设定"
                        } else {
                            "--"
                        },
                        style = MaterialTheme.typography.bodyLarge,
                    ...
                    Row(
                        ...
                        listOf(0, 15, 30).forEach { minute ->
                            SelectableActionButton(
                                text = if (minute == 0) "不定时" else "${minute}分钟",
                                selected = timerMinState == minute,
                                modifier = Modifier.weight(1f),
                                onClick = { updateTimerMin(minute) }
                            )
                        }
                    }
                    Row(
            ...
            HomeSection(title = "电源控制") {
                Column(
                    ...
                    Button(
                        ...
                        Text(text = if (isRunning) "关机" else "开机")
                    }
                }
            }

            HomeSection(title = "设备状态") {
                Column(
                    modifier = Modifier
                        .fillMaxWidth()
                        .padding(horizontal = 16.dp, vertical = 14.dp),
                    ...
                    ProtoFieldRow(
                        label1 = "超温告警",
                        value1 = if (hasBleConnection) {
                            if (isOverTemp) "⚠ 超温" else "正常"
                        } else {
                            "--"
                        },
                        label2 = "干烧告警",
                        value2 = if (hasBleConnection) {
                            if (isDryBurn) "⚠ 缺水" else "正常"
                        } else {
                            "--"
                        },
                        value1Color = if (hasBleConnection && isOverTemp) {
                            MaterialTheme.colorScheme.error
                        } else {
                            MaterialTheme.colorScheme.onSurface
                        },
                        value2Color = if (hasBleConnection && isDryBurn) {
                            MaterialTheme.colorScheme.error
                        } else {
                            MaterialTheme.colorScheme.onSurface
                        }
                    ...
                    ProtoFieldRow(
                        label1 = "故障码",
                        value1 = if (hasBleConnection) faultCode.toString() else "--",
                        label2 = "故障状态",
                        value2 = if (hasBleConnection) faultText else "--",
                        value2Color = if (hasBleConnection && faultCode != 0) {
                            MaterialTheme.colorScheme.error
                        } else {
                            MaterialTheme.colorScheme.onSurface
                        }
                    )
                    if (hasBleConnection && faultCode != 0) {
                        Text(
                            text = "故障：$faultText",
                            style = MaterialTheme.typography.bodyLarge,
    ...
    val disconnectedHint = if (UserConfig.DEVELOPER_MODE) {
        "未连接 BLE 设备，界面数据显示为 --。"
    } else {
        "未连接 BLE 设备，界面数据显示为 --，下拉可重新搜索设备。"
    }

    val screenContent: @Composable (Modifier) -> Unit = { containerModifier ->
        Column(
