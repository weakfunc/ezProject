...
- **项目名称**（显示在 APP 主页顶部）：宠物智能饲喂系统设计与实现
- **项目作者**（显示在 APP 主页顶部）：韦孙麟
...
```
投喂通过控制360度舵机开关时间实现，舵机顺时针转动带动食槽打开，称重传感器称重，当重量达到预设值附近（比如100g，则重量传感器反馈90g时）逆时针转动关闭食槽，完成一次投喂。
重量传感器10ms周期检测重量，余量不足报警功能由板载蜂鸣器实现，报警阈值通过APP设置。
APP设置投喂方案有以下用户配置项：1.方案id 2.投喂频率（单位s，比如30s，60s）2.投喂重量（单位g，比如30g，50g），stm32端应该有一个“投喂计划”列表保存上述参数并进行执行，同时每次投喂后存储到“历史投喂数据”列表，并发给APP。
APP可以控制是否执行投喂计划，执行投喂计划则按计划控制舵机。未执行投喂计划时，APP可以控舵机打开与关闭。
需要保存flash实现断电记忆功能，STM32上电应该先从FLASH读取投喂计划，并同步给APP，投喂计划的增/删/改权限只有APP，stm32端只负责保存和执行。
除此之外STM32还需要反馈的信息：投喂状态（舵机状态），当前食槽重量，当前投喂计划，系统故障（当BLE未连接时显示系统断电故障，其余情况均显示系统正常）

系统上电，默认先执行投喂方案1，rfid识别到宠物id后，执行对应id的投喂方案，APP端也可以选择投喂方案，APP端优先级大于stm32端的rfid识别

对于stm32向APP发送flash的投喂计划列表以及APP向stm32发送用户对投喂计划列表的增/删/改可能需要你思考一下在当前协议架构下如何最简单实现。
```
