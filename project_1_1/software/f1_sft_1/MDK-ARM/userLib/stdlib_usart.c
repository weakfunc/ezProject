...
static void __STDLIB_USART_DispatchRxByte(uartRawInfo_t *c, uint8_t byte){
    if(c->cfg.customProtocol && c->parseCb != NULL){
    ...
    for(uint8_t i = 0U; i < PORT_MAX; i++){
        ...
        if(__STDLIB_USART_IsRxDma(c->huart->Instance)){
            ...
            while(c->dmaTail != dmaHead){
                __STDLIB_USART_DispatchRxByte(c, c->dmaBuf[c->dmaTail]);
        ...
        while(__STDLIB_USART_RxPop(c, &byte) == 0){
            __STDLIB_USART_DispatchRxByte(c, byte);
        }
    }
}
