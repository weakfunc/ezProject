...
![image-20260108204948086](C:\Users\权恒毅\AppData\Roaming\Typora\typora-user-images\image-20260108204948086.png)
...
![image-20260108205043896](C:\Users\权恒毅\AppData\Roaming\Typora\typora-user-images\image-20260108205043896.png)

​		系统上电后首先完成单片机及各外设的初始化，并对超声波模块1进行零点位置标定，以建立液位测量的参考基准，随后通过按键完成人工设置液位报警阈值等参数，在系统运行过程中同步采集超声波模块2和模块3的测距原始数据，结合两模块已知的安装高度差对多路测量结果进行融合计算，以减小声速变化、回波衰减及液面扰动对测量精度的影响，融合后的液位高度结果用于计算液位变化量并与设定阈值进行比较，当判断液位超出阈值时通过IO口控制声光报警模块进行报警提示，同时将当前液位数据及系统工作状态通过SPI接口显示在OLED屏幕上，完成一次完整的测量与判断流程并进入下一周期运行。
...
（3）超声波测距模块：本系统选用电应普科技 DYP-A22-V1.0 超声波测距模块作为液位检测核心传感器。该模块采用一体化设计，内部集成超声波发射、接收及信号处理电路，支持 UART 等多种通信方式，测量范围可覆盖 1～300 cm，响应速度快，适用于中小型容器的非接触式液位测量。

（4）上位机 APP：采用 App Inventor 开发，通过蓝牙与下位机通信，实现液位高度监控的显示、数据记录及运行状态监控，结构简单、维护方便，满足系统监控与管理需求。
