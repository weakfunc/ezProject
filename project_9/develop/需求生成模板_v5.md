...
```
在下位机上使用OLED模块显示所需要的信息，实现功能即可，对于具体的指标所选硬件性能均能够达到。报警功能先不实现，后续我会补充实现。
该液位高度测量装置装在水箱顶部，包含两个超声波测距模块。高精度超声波测距负责测量模块距液面的距离，普通超声波传感器负责测量模块距地面的距离。距地面距离-距液面距离=液位高度。模块上电时，先采集几次普通超声波测距模块的数值获得距地面的距离，从而计算出液面高度。
```
