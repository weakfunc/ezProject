...
static void packRecognize(void){
  ...
  *pHasNew = 0U;
  ...
  if(user1TaskInfo.packNum >= PACK_MAX_NUM){
	...
	remoteInfo.var[0] = user1TaskInfo.packNum;
	remoteInfo.var[1] = user1TaskInfo.packList[user1TaskInfo.packNum].isOutFlag;
	remoteInfo.var[2] = user1TaskInfo.packList[user1TaskInfo.packNum].servoId;
...
void user1TaskUpdata(void *argument){
	...
	for(;;){
		...
		remoteInfo.var[3] = systemTaskInfo.key1Press;
