...
void systemTaskUpdata(void *argument){
  ...
  for(;;){
	  ...
	  if(DRIVER_BOARD_KeyIsPressed(BOARD_KEY1) || remoteInfo.remoteVar_RX[0].var_uint32){
			systemTaskInfo.key1Press = 1;
		}
		
	  if(systemTaskInfo.key1Press == 1 && !remoteInfo.remoteVar_RX[0].var_uint32){
			systemTaskInfo.key1Press = 0;
		}

		if(DRIVER_BOARD_KeyIsPressed(BOARD_KEY2) || remoteInfo.remoteVar_RX[1].var_uint32){
