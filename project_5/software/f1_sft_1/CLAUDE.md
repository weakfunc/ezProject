...
- 新建driver_DS3231RTC模块，该模块为时钟模块，使用I2C通信，默认使用I2C_BUS_1
- 在ds3231RTCInfo_t中储存时间信息
- 在usertask每秒更新时间

## 硬件确认（已知）
- 模块型号: A7670C（支持2G/3G/4G全网通）
- 通信接口: USART1，波特率115200
- AT指令集: 标准SIM7670系列兼容指令

## 短信实现要求
- 使用PDU模式（AT+CMGF=0），支持中文内容（UCS2编码）
- 实现UTF-8转UCS2编码（用于中文短信内容）
- 短信中心号码留空（使用SIM卡默认值，填00）
- AT指令等待响应超时时间可配置，默认5000ms
- UART接收使用中断+静态环形缓冲区，不使用阻塞接收
